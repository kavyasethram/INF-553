...
if __name__ == "__main__":
    ...
    global_frequent_set = [tuple(l) for l in global_frequent_set]
    ind_globl_freq_set = []
    rem_globl_freq_set = []
    maxlen =  max(map(len,global_frequent_set))
    for i in range(0,len(global_frequent_set)):
    ...
    for frequent_items in ind_globl_freq_set:
        output_file.write(str(frequent_items).replace("(","").replace(")","").replace(",",""))
        output_file.write("\n")
    for i in range(2,maxlen+1):
        for freq_rw in rem_globl_freq_set:
            if len(freq_rw) == i:
                #file_writer.writerow(freq_rw)
                output_file.write(str(freq_rw))
                output_file.write("\n")
    output_file.close()
